...
from magicrl.agents.offline import IQLAgent
...
if __name__ == '__main__':
    ...
    parser.add_argument('--learn_id', type=str, default='iql/hop-m-v2')
    ...
    actor = GaussionActor(obs_dim=obs_dim, act_dim=act_dim, act_bound=act_bound, hidden_size=[256, 256], logstd_min=-6.0, logstd_max=0.0, logstd_hard_clip=False)
    critic_q1 = SimpleCritic(obs_dim=obs_dim, act_dim=act_dim, hidden_size=[256, 256])
    critic_q2 = SimpleCritic(obs_dim=obs_dim, act_dim=act_dim, hidden_size=[256, 256])
    critic_v = SimpleCritic(obs_dim=obs_dim, act_dim=0, hidden_size=[256, 256])

    agent = IQLAgent(actor=actor,
                     critic_q1=critic_q1,
                     critic_q2=critic_q2,
                     critic_v=critic_v,
                     actor_lr=3e-4,
